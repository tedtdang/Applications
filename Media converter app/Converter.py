...
class Media:
    """
    Convert from videos to audio or audio to other extensions
    Adjust media speed, duration, and more
    """
    ...
    def to_second(time_string: str) -> Union[int, float]:
        ...
        ----------
        time_string : str
            The time string to parse, in the format "xx:xx(:xx)"
        Returns
        -------
        Union[int, float]
            The number of seconds represented by the time string
        """
        struct_time = time.strptime(time_string, '%H:%M:%S' if ':' in time_string else '%M:%S')
